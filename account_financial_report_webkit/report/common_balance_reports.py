...
class CommonBalanceReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def original_compute_balance_data(self, data, filter_report_type=None):
    ...
    def compute_balance_data(self, data, filter_report_type=None):
        ...
        ids_with_analytic_accounts = dict.fromkeys(account_ids, False)
        code_with_analytic_accounts = dict.fromkeys(account_ids, False)
        debit_with_analytic_accounts = dict.fromkeys(account_ids, False)
        credit_with_analytic_accounts = dict.fromkeys(account_ids, False)
        balance_with_analytic_accounts = dict.fromkeys(account_ids, False)
        init_balance_with_analytic_accounts = dict.fromkeys(account_ids, False)

        for account in objects:
            ids_with_analytic_accounts[account.id] = []
            code_with_analytic_accounts[account.id] = []
            debit_with_analytic_accounts[account.id] = []
            credit_with_analytic_accounts[account.id] = []
            balance_with_analytic_accounts[account.id] = []
            init_balance_with_analytic_accounts[account.id] = []
        ...
        if not (start_date and stop_date):
            if start_period and stop_period:
                start_date = start_period.date_start#.strftime('%Y-%m-%d')
                stop_date = stop_period.date_stop#.strftime('%Y-%m-%d')
        row_ids = self.get_details_from_move_line_with_analytic_accounts(account_ids, start_date, stop_date, target_move)
        for row in row_ids :
            ids_with_analytic_accounts[row['a_id']].append(row['al_id'])
            code_with_analytic_accounts[row['a_id']].append(row['al_code'])
            debit_with_analytic_accounts[row['a_id']].append(row['aml_debit'])
            credit_with_analytic_accounts[row['a_id']].append(row['aml_credit'])
            balance_with_analytic_accounts[row['a_id']].append(row['aml_balance'])
            init_balance_with_analytic_accounts[row['a_id']].append(row['init_balance'])
        context_report_values = {
            'fiscalyear': fiscalyear,
            'start_date': start_date,
            'stop_date': stop_date,
            ...
            'credit_accounts': credit_accounts,
            'balance_accounts': balance_accounts,
            'ids_with_analytic_accounts': ids_with_analytic_accounts,
            'code_with_analytic_accounts': code_with_analytic_accounts,
            'debit_with_analytic_accounts': debit_with_analytic_accounts,
            'credit_with_analytic_accounts': credit_with_analytic_accounts,
            'balance_with_analytic_accounts': balance_with_analytic_accounts,
            'init_balance_with_analytic_accounts': init_balance_with_analytic_accounts
        }

        return objects, new_ids, context_report_values
    
    def get_details_from_move_line_with_analytic_accounts(self, account_ids, start_date=False, stop_date=False, target_move="all"):
        res = False
        first = True
        DATE_CLAUSE = """"""
        if start_date and stop_date :
            DATE_CLAUSE = """ AND aml.date <= '"""+stop_date+"""'
            AND aml.date >= '"""+start_date+"""' """
        
        DATE_INIT_CLAUSE = """"""
        if start_date and stop_date :
            DATE_INIT_CLAUSE = """ AND aml.date < '"""+start_date+"""' """
        ...
        if target_move == "posted":
            TARGET_MOVE_CLAUSE = """ AND am.state = 'posted' """
        ...
        query = """
            SELECT 
                ...
                (SUM(aml.debit)-SUM(aml.credit)) as aml_balance
            FROM account_move_line aml
            LEFT JOIN account_account a ON (aml.account_id = a.id)
            LEFT JOIN account_analytic_account al ON (aml.analytic_account_id = al.id)
        ...
        try:
            ...
            self.cursor.execute(query_init)
            res_init = self.cursor.dictfetchall()
            for r in res:
                r['init_balance'] = 0
                for r_init in res_init:
                    if r['a_id']==r_init['a_id'] and r['al_id']==r_init['al_id']:
                        r['init_balance'] = r_init['aml_balance']
        except Exception:
