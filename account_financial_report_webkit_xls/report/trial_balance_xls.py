...
#             _p.initial_balance_mode], None, cell_style_center),
        ...
        if _p.comparison_mode == 'no_comparison' or not _p.fiscalyear:
            c_specs += [('balance', 1, 0, 'text',
                         _('Ending Balance'), None, cell_style_right)]
        else:
        ...
        for current_account in objects:
            ...
            al_ids = _p['ids_with_analytic_accounts'][current_account.id]
            iterator = 0
            add_code = "0000"
            sub_debit = 0
            sub_credit = 0
            ...
            sub_init_balance = 0
            for al_id in al_ids:
                sub_debit += _p['debit_with_analytic_accounts'][current_account.id][iterator]
                sub_credit += _p['credit_with_analytic_accounts'][current_account.id][iterator]
                sub_balance += _p['balance_with_analytic_accounts'][current_account.id][iterator]
                sub_init_balance += _p['init_balance_with_analytic_accounts'][current_account.id][iterator]
                iterator += 1
            ...
            c_specs = [
                ('code', 1, 0, 'text', current_account.code + '.' + add_code),
            ...
            if _p.comparison_mode == 'no_comparison':
                ...
                if _p.initial_balance_mode:
                    ...
                    c_specs += [('init_bal', 1, 0, 'number',
                                 _p['init_balance_'
                                    'accounts'][current_account.id] - sub_init_balance,
                                 None,
                                 cell_style_decimal)]
                c_specs += [
                    ('debit', 1, 0, 'number',
                     _p['debit_accounts'][current_account.id] - sub_debit,
                     None, cell_style_decimal),
                    ('credit', 1, 0, 'number',
                     _p['credit_accounts'][current_account.id] - sub_credit,
                     None, cell_style_decimal),
                ]
                ...
                c_specs += [('balance', 1, 0, 'number',
                             _p['balance_accounts'][current_account.id] - sub_balance,
                             None, cell_style_decimal)]
            ...
            if add_code == "0000" and current_account.user_type.report_type in ['income', 'expense']:
                c_specs = []
            if c_specs:         
                row_data = self.xls_row_template(c_specs, [x[0] for x in c_specs])
                row_pos = self.xls_write_row(
            ...
            for al_id in al_ids:
                ...
                c_specs = [
                    ('code', 1, 0, 'text', current_account.code + '.' + _p['code_with_analytic_accounts'][current_account.id][iterator]),
                    ('account', account_span, 0, 'text', current_account.name),
                ]
                if _p.comparison_mode == 'no_comparison':
                    ...
                    if _p.initial_balance_mode:
                        ...
                        bal_formula = init_cell + '+' + \
                            debit_cell + '-' + credit_cell
                        c_specs += [('init_bal', 1, 0, 'number',
                                     _p['init_balance_with_analytic_accounts'][current_account.id][iterator],
                                     None,
                                     cell_style_decimal)]
                    c_specs += [
                        ('debit', 1, 0, 'number',
                         _p['debit_with_analytic_accounts'][current_account.id][iterator],
                         None, cell_style_decimal),
                        ('credit', 1, 0, 'number',
                         _p['credit_with_analytic_accounts'][current_account.id][iterator],
                         None, cell_style_decimal),
                    ]
                    c_specs += [('balance', 1, 0, 'number', None,
                                 bal_formula, cell_style_decimal)]
                    ...
                    c_specs += [('balance', 1, 0, 'number',
                                 _p['balance_with_analytic_accounts'][current_account.id][iterator],
                                 None, cell_style_decimal)]
